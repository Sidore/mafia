...
let clients = {
    ...
    findUser : function(message) {
        return clients.users.find(u => u.name == message);
    },
    findUserByRole : function(role) {
        return clients.users.find(u => u.role == role);
    },
    findUsers : function(){
    ...
    broadcast : function(message, role = "*"){
        if (role == "alive") {
            this.users.filter(u =>  u.status != "dead").map(u => u.ws).forEach(u => u.send(message));            
        } else {
            this.users.filter(u => role == "*" || u.role == role).map(u => u.ws).forEach(u => u.send(message));
        }
    },
    send : function(){}
};
...
let game = {
    ...
    isFinnished : function(){
        return !(this.clients.users.some(u => u.status != "dead" && (u.role == "civil" || u.role == "doctor")) 
    ...
    whoWon : function(){
        return this.clients.users.some(u => u.status != "dead" && (u.role == "civil" || u.role == "doctor")) ? "civil" : "mafia";
    },
    checkRoles : function(){
    ...
    events : {
        ...
        setRoles : function(){
            ...
            console.log(game.clients.users.map(u => {return {"name" : u.name, "role" : u.role}}));
        ...
        mafia : function(message){
            ...
            // console.log(message, )
            // game.state = game.states[4];
            let d = clients.users.find(u => u.name == message);
            d.status = "dead";
        ...
        police : function(message,user){
            ...
            var ans = d.role == "mafia" ? "мафия!" : "не мафия"
            // game.clients.broadcast("Ночью был убит " + d.name);
        ...
        voting : function(message, user){
            if (!game.voting[user.name]) {
                ...
                var goal = game.clients.findUser(message)
                if (goal) {
            ...
            if (game.voting._length === game.clients.users.filter(u => u.status != "dead").length) {
        ...
        voted : function(){
            ...
            game.clients.users.filter(u => u.status != "dead").forEach(u => {
                var name = u.name;
                var vote = game.voting[name];

                var user = results.find(u => u.name == vote);
...
wss.on("connection", (ws) => {
    ...
    console.log(clients.users.map(u => u.name));
